...
export function Orders() {
  ...
  async function handleCreate(e) {
    e.preventDefault();
    try { await api.createOrder(form); toast(t("ui.order_created")); setShowModal(false); }
  ...
  return (
    <div className="page">
      ...
      <div className="page-header flex-between">
        <div><h1>{t("ui.it_orders")}</h1><p>{isTech ? t("ui.0_orders_in_queue", orders.length) : t("ui.submit_a_tech_support_order")}</p></div>
      ...
      {isTech && (
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead><tr><th>{t("ui.device")}</th><th>{t("ui.description")}</th><th>{t("ui.user")}</th><th>{t("ui.status")}</th><th>{t("ui.actions")}</th></tr></thead>
              <tbody>
                {orders.map(o => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <div style={{ display:"flex", gap:6 }}>
                        {o.status === "PENDING"   && <button className="btn btn-primary btn-sm" onClick={() => handleAccept(o.id)}>{t("ui.accept")}</button>}
                        {o.status === "ACCEPTED"  && <button className="btn btn-success btn-sm" onClick={() => handleComplete(o.id)}>{t("ui.complete")}</button>}
                      </div>
                    </td>
                  </tr>
                ))}
                {orders.length === 0 && <tr><td colSpan="5" style={{ textAlign:"center", color:"var(--text-2)" }}>{t("ui.no_orders")}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {showModal && (
        <Modal title={t("ui.create_it_order")} onClose={() => setShowModal(false)}
          ...
          <form id="order-form" onSubmit={handleCreate} style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div className="form-group" style={{ marginBottom:0 }}><label>{t("ui.device_type")}</label>
